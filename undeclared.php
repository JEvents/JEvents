...
$deprecatedIdentifiers = [
    ...
    // Special case - Joomla has 2 classes with the same name!
    'StringHelper'       => array(
...
function searchDeprecatedFunctions($filePath, $deprecatedIdentifiers) {
    ...
    foreach ($deprecatedIdentifiers as $identifierxxx => $alias)
    {
        if ( is_array( $alias ) )
        {
            $identifier = $identifierxxx;
        }
        else
        {
            $namespacedClass = explode("\\", $alias);
            $identifier = $namespacedClass[count($namespacedClass) - 1];
        }

        // Create a regex pattern to match the identifier in various contexts
        $pattern = '/\b(\w+::' . preg_quote($identifier, '/')
                   . '|\b' . preg_quote($identifier, '/') . '::'
                   . '|\bnew\s+' . preg_quote($identifier, '/')
                   . '|\bextends\s+' . preg_quote($identifier, '/') . ')\b/';

        if (preg_match($pattern, $fileContents)) {

            //echo "Namespaced method '$identifier' found in file: $filePath\n";
            //echo "looking for '$alias'\n";

            $matched = false;
            foreach ( (array) $alias as $aliasItem )
            {
                if (!$matched && stripos($fileContents, $aliasItem) !== false)
                {
                    $matched = true;
                }
            }
            if (!$matched)
            {
                echo "Namespaced method without use statement '$identifier' found in file: $filePath\n";
            }
        }
    }
}
