		...
		if (($jevuser && $jevuser->candeleteall) || $access)
		{
			...
			if (!isset($users))
			{
				...
				if ($userCount>$maxDirectNumber) {
					...
					$datapath = JRoute::_("index.php?option=com_jevents&task=gwejson&file=findcreator", false);
					//$prefetchdatapath = JRoute::_("index.php?option=com_jevents&task=gwejson&file=findcreator&prefetch=1", false);
