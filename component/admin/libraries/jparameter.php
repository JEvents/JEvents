...
class JParameter extends JRegistry
{
	...
	public function setXML(&$xml)
	{
		...
		if (is_object($xml))
		{
			if ($group = $xml->getAttribute('group'))
			{
				$this->_xml[$group] = $xml;
			}
			else
			...
			if ($dir = $xml->getAttribute('addpath'))
			{
				$this->addElementPath(JPATH_ROOT );
			}
		}
	}
	...
	public function render($name = 'params', $group = '_default')
	{
		...
		if ($description = $this->_xml[$group]->getAttribute('description'))
		{
	...
	public function getParam(&$node, $control_name = 'params', $group = '_default')
	{
		...
		// Get value.
		$value = $this->get($node->getAttribute('name'), $node->getAttribute('default'), $group);
