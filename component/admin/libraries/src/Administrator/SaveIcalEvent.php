...
/**
 * Saves an iCal event (vevent + repetitions) from form data.
 * Legacy class name: SaveIcalEvent
 */
class SaveIcalEvent
{
	...
	public static function save($array, &$queryModel, $rrule, $dryrun = false)
	{
		...
		// Joomla 3.2 fix — the form doesn't respect the checkbox value in the form xml being "on" instead of 1
		...
		// If user is jevents candeleteall or has backend access then allow them to specify the creator
		...
		if (!$dryrun && $success && $notifyAdmin)
		{
			...
			// reload the event to get the repetition ids
			$evid      = (int) $vevent->ev_id;
			$testevent = $queryModel->getEventById($evid, 1, "icaldb");

			if (!$testevent)
			{
				// Could be that the event cannot be displayed on this menu item; disable category access checks
