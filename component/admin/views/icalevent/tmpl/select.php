		...
		for ($i = 0, $n = count($this->rows); $i < $n; $i++)
		{
			...
			*/
			...
			<tr class="row<?php echo $k; ?>">
				<td >
					<a href="#select" onclick="return window.parent.jSelectEvent('<?php echo $link;?>','<?php echo addslashes($repeat->title());?>' , ($('Itemid')?$('Itemid').value:0) , <?php echo $repeat->rp_id();?>, <?php echo $repeat->ev_id();?>)" title="<?php echo JText::_('JEV_SELECT_EVENT'); ?>"><?php echo $row->title(); ?></a>
				</td>
				<td align="center">
					<?php
					if ($row->hasrepetition())
					{
						...
						?>
						<a href="<?php echo JRoute::_("index.php?option=com_jevents&tmpl=component&task=icalrepeat.select&evid=".$row->ev_id()."&".JSession::getFormToken().'=1&nomenu='.JRequest::getInt("nomenu")); ?>" title="<?php echo JText::_("JEV_SELECT_REPEAT"); ?>" >
						<?php echo $img; ?>
						</a>
					<?php } ?>
				</td>
