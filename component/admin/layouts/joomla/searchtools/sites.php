...
if (isset($data['view']->filterForm) && !empty($data['view']->filterForm))
{
	...
	// Checks if it should show the be hidden - either no filters of JUST coretype
	$hideActiveFilters = empty($data['view']->activeFilters) || (count($data['view']->activeFilters) == 1 && isset($data['view']->activeFilters['coretype']));
...
<div class="ysts_list_filters" gsl-grid>
        ...
        {
	        $filters = $data['view']->filterForm->getGroup('filter');
	        $field   = isset($filters['filter_coretype']) ? $filters['filter_coretype'] : false;
	        if ($field)
	        {
		        ?>
                <div class="js-stools-field-filter">
			        <?php
                    $coretypeinput = $field->input;
                    echo $coretypeinput;
                    /*
			        $htmlDoc = new DomDocument();

			        $htmlDoc->loadHTML($coretypeinput);
			        $inputs = $htmlDoc->getElementsByTagName('input');
			        foreach ($inputs as $i => $input)
			        {
				        $inputs[$i]->setAttribute('class', $inputs[$i]->getAttribute('class') . " gsl-radio");
			        }

			        $inputs = $htmlDoc->getElementsByTagName('label');
			        foreach ($inputs as $i => $input)
			        {
				        $inputs[$i]->setAttribute('class', $inputs[$i]->getAttribute('class') . " gsl-button");
				        if (trim($inputs[$i]->nodeValue) == 'Joomla'){
					        $inputs[$i]->setAttribute('class', $inputs[$i]->getAttribute('class') . " gsl-button-primary");
				        }
			        }

                    echo $htmlDoc->saveHTML($htmlDoc->getElementById('filter_coretype'));
                    */
                    ?>
                </div>
		        <?php
	...
	<div class="js-stools-container-filters gsl-child-width-1-4@s gsl-child-width-1-6@m <?php echo $filtersActiveClass; ?> gsl-child-width-expand@l " gsl-grid>
