 ...
 *
 * @version     $Id: iCalImport.php 1729 2010-03-25 03:01:50Z geraint $
 * @package     JEvents
...
class iCalImport
{
	...
	function add_to_cal($parent, $key, $value, $append)
	{
		...
		if (stristr($key,"DTSTART") || stristr($key,"DTEND") || stristr($key,"EXDATE")) {
			...
			if (stristr($key,"DTEND") == "DTEND" && strlen($rawvalue)>=15 && substr($rawvalue,9,6)=="000000") {
				$value -= 1;  // 1 second
				//$value -= 86400;  // 1 day
			}
		...
		switch ($parent)
		{
			...
			case "VEVENT":
				...
				// Strip http:// from UID !!!
				if ($key=="UID" && strpos($value,"http://")!==false){
