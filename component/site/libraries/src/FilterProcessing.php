...
class FilterProcessing
{
	...
	public static function & getInstance($item, $filterpath = "", $unsetfilter = false, $uid = "")
	{
		...
		{
			$instances[$key] = new static($item, $filterpath);
		}
...
class jevFilter
{
	...
	function __construct($tablename, $filterfield, $isString = false)
	{
		...
		{
			$module = \Joomla\CMS\Helper\ModuleHelper::getModule("mod_jevents_filter");
		}
		if ($module && $module->id)
		{
			$modparams = new \JevRegistry($module->params);
	...
	function &getSession()
	{
		...
		if (!isset($session))
		{
			include_once(JEV_LIBS . "/Session.php"); // TODO: Session.php / gweSession not found in codebase
...
class jevBooleanFilter extends jevFilter
{
	...
	public function _createfilterHTML()
	{
		...
		$options             = array();
		$options[]           = \Joomla\CMS\HTML\HTMLHelper::_('select.option', "-1", $this->bothLabel, "value", "yesno");
		$options[]           = \Joomla\CMS\HTML\HTMLHelper::_('select.option', "0", $this->noLabel, "value", "yesno");
		$options[]           = \Joomla\CMS\HTML\HTMLHelper::_('select.option', "1", $this->yesLabel, "value", "yesno");
		$filterList["html"]  = \Joomla\CMS\HTML\HTMLHelper::_('select.genericlist', $options, $this->filterType . '_fv', 'class="inputbox" size="1" onchange="form.submit();"', 'value', 'yesno', $this->filter_value);
	...
	public function _createfilterHtmlUIkit()
	{
		...
		$options[]           = \Joomla\CMS\HTML\HTMLHelper::_('select.option', "0", $this->noLabel, "value", "yesno");
		$options[]           = \Joomla\CMS\HTML\HTMLHelper::_('select.option', "1", $this->yesLabel, "value", "yesno");
		$filterList["html"]  = \Joomla\CMS\HTML\HTMLHelper::_('select.genericlist', $options, $this->filterType . '_fv', 'class="uk-select uk-form-width-medium" size="1" onchange="form.submit();"', 'value', 'yesno', $this->filter_value);
