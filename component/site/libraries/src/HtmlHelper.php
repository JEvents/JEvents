...
use Joomla\CMS\HTML\HTMLHelper as JoomlaHTMLHelper;
...
class HtmlHelper
{
	public static function buildReccurDaySelect($reccurday, $tag_name, $args)
	{
		...
		$daynamelist[] = JoomlaHTMLHelper::_('select.option', '-1', '&nbsp;' . Text::_('JEV_BYDAYNUMBER') . '<br />');

		for ($a = 0; $a < 7; $a++)
		{
			$name_of_day = '&nbsp;' . $day_name[$a]; //getDayName($a);
			$daynamelist[] = JoomlaHTMLHelper::_('select.option', $a, $name_of_day);
		}

		$tosend = static::buildRadioOption($daynamelist, $tag_name, $args, 'value', 'text', $reccurday);
		echo $tosend;
	}
	...
	public static function buildMonthSelect($month, $args)
	{
		for ($a = 1; $a < 13; $a++)
		{
			...
			$monthslist[] = JoomlaHTMLHelper::_('select.option', $mnh, $name_of_month);
		}

		$tosend = JoomlaHTMLHelper::_('select.genericlist', $monthslist, 'month', $args, 'value', 'text', $month);
		echo $tosend;
	}
	...
	public static function buildDaySelect($year, $month, $day, $args)
	{
		...
		{ //32
			...
			$dayslist[] = JoomlaHTMLHelper::_('select.option', $dys, $dys);
		}

		$tosend = JoomlaHTMLHelper::_('select.genericlist', $dayslist, 'day', $args, 'value', 'text', $day);
		echo $tosend;
	}
	...
	public static function buildYearSelect($year, $args)
	{
		...
		{
			$yearslist[] = JoomlaHTMLHelper::_('select.option', $year, $year);
		}
		...
		for ($i = $y - 2; $i <= $y + 5; $i++)
		{
			...
			{
				$yearslist[] = JoomlaHTMLHelper::_('select.option', $i, $i);
			}
		}
		...
		$tosend = JoomlaHTMLHelper::_('select.genericlist', $yearslist, 'year', $args, 'value', 'text', $year);
		echo $tosend;
	}
	...
	public static function buildViewSelect($viewtype, $args)
	{
		...
		if (in_array("byday", $iconstoshow))
			$viewlist[] = JoomlaHTMLHelper::_('select.option', 'day.listevents', Text::_('JEV_VIEWBYDAY'));
		if (in_array("byweek", $iconstoshow))
			$viewlist[] = JoomlaHTMLHelper::_('select.option', 'week.listevents', Text::_('JEV_VIEWBYWEEK'));
		if (in_array("bymonth", $iconstoshow))
			$viewlist[] = JoomlaHTMLHelper::_('select.option', 'month.calendar', Text::_('JEV_VIEWBYMONTH'));
		if (in_array("byyear", $iconstoshow))
			$viewlist[] = JoomlaHTMLHelper::_('select.option', 'year.listevents', Text::_('JEV_VIEWBYYEAR'));
		if (in_array("bycat", $iconstoshow))
			$viewlist[] = JoomlaHTMLHelper::_('select.option', 'cat.listevents', Text::_('JEV_VIEWBYCAT'));
		if (in_array("search", $iconstoshow))
			$viewlist[] = JoomlaHTMLHelper::_('select.option', 'search.form', Text::_('JEV_SEARCH_TITLE'));

		$tosend = JoomlaHTMLHelper::_('select.genericlist', $viewlist, 'task', $args, 'value', 'text', $viewtype);
		echo $tosend;
	}
	...
	public static function buildHourSelect($start, $end, $inc, $tag_name, $tag_attribs, $selected, $format = '')
	{
		...
		for ($i = $start; $i <= $end; $i += $inc)
		{
			...
			$arr[] = JoomlaHTMLHelper::_('select.option', $fi, $tmpi);
		}

		return JoomlaHTMLHelper::_('select.genericlist', $arr, $tag_name, $tag_attribs, 'value', 'text', $selected);
	}
	...
	{
		// need to declare this because of bug in Joomla JoomlaHTMLHelper::_('select.options', on content pages - it loade the WRONG CLASS!
        ...
        foreach ($items as &$item)
        {
            ...
            $option = JoomlaHTMLHelper::_('select.option', $item->id, $item->title);
		...
		if ($allowMultiCat || ($eventediting && $params->get("multicategory", 0)))
		{
			...
			?>
			<?php echo JoomlaHTMLHelper::_('select.options', $options, $optionParameters); ?>
		    </select>
		    <?php
		    $html = ob_get_clean();
		    if (count($options) == 1  && $jevtask !== "defaults.edit")
			    ...
			    if (!isset($arr_userids[$userid]))
			    {
				    ...
				    if ($userdet)
				    {
					    ...
					    else if ($userdet->email && $agenda_viewmail == '1')
					    {
						    ...
						    {
							    $contactlink = JoomlaHTMLHelper::_('email.cloak', $userdet->email, 1, $userdet->name, 0);
						    }
						    else
						    {
							    $contactlink = JoomlaHTMLHelper::_('email.cloak', $userdet->email, 1, $userdet->username, 0);
						    }
					    }
					    else
					    {
			    ...
			    if (!isset($arr_evids[$evid]))
			    {
				    ...
				    if ($anonplugin)
				    {
					    ...
					    if ($anonrow)
					    {
						    ...
						    {
							    $contactlink = JoomlaHTMLHelper::_('email.cloak', $anonrow->email, 1, $anonrow->name, 0);
						    }
						    else
						    {
	    ...
	    {
		    $assetGroups = JoomlaHTMLHelper::_('access.assetgroups');
			...
			echo JoomlaHTMLHelper::_('select.options', $assetGroups, 'value', 'text', $access);
			?>
		    </select>
		    <?php
		    return ob_get_clean();
