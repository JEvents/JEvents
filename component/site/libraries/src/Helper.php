...
class Helper
{
	...
	function getMinYear()
	{
		...
		$minyear = self::getYearNumber($minyear);
	...
	function checkRobotsMetaTag($name = "robots", $content = "index,follow")
	{
		...
		if ($cfg->get('com_blockRobots', 0) >= 1)
		{
			...
			// conditional on date
			list($cyear, $cmonth, $cday) = self::getYMD();
...
*/
	...
	function loadElectricCalendar($fieldname, $fieldid, $value, $minyear, $maxyear, $onhidestart = "", $onchange = "", $format = 'Y-m-d', $attribs = array(), $showtime = false, $showDefaultDateValue = true)
	{
		...
		if (version_compare(JVERSION, '4.0.0', '>='))
		{
            ...
            $format = self::mapStrftimeFormatToDateFormat($format);
            ...
            echo self::j4calendar($value, $fieldname, $fieldid, $format, $attributes);
		...
		if (!isset($jevitemid))
		{
			...
			else
			{
				...
				if (count($jevitems) > 0)
				{
					...
					{
						if (in_array($jevitem->access, self::getAid($user, 'array')))
						{
	...
	function getAid($user = null, $type = 'string')
	{
		...
		else
		{
			...
			if (self::isAdminUser($user) && Factory::getApplication()->isClient('administrator'))
			{
			...
			if (is_null($user))
			{
				...
				if (!$authorisedonly)
				{
					...
					if ($params->get("category_allow_deny", 1) == 0)
					{
						...
						if ($juser->get("id"))
						{
							$okcats = self::getAuthorisedCategories($juser, 'com_jevents', 'core.create');
							$juser  = Factory::getUser();
							if (count($okcats))
							{
				...
				else if ($juser->id > 0 && self::isAdminUser($juser))
				{
	...
	function canCreateEvent($row, $user = null)
	{
		...
		if (!$authorisedonly)
		{
			if ($user->authorise('core.create', 'com_jevents'))
				return true;
			$allowedcats = self::getAuthorisedCategories($user, 'com_jevents', 'core.create');
			...
			if ($user->id == 0 && count($allowedcats) == 0)
			{
				...
				if (strpos($jevtask, "icalevent.save") !== false || strpos($jevtask, "icalevent.apply") !== false)
				{
					...
					$catids     = self::rowCatids($row) ? self::rowCatids($row) : array(intval($row->_catid));
			...
			if (self::rowCatids($row))
			{
				if (count(array_diff(self::rowCatids($row), $allowedcats)))
				{
					return false;
				}
			}
		...
		else
		{
			...
			$jevuser = self::getAuthorisedUser();
	...
	function canEditEvent($row, $user = null)
	{
		...
		if (self::isEventEditor())
		{
			// any category restrictions on this?
			// This involes TOO many database queries in Joomla - one per category which can be a LOT
			/*
			  $cats = self::getAuthorisedCategories($user,'com_jevents', 'core.edit');
			  $cats_own = self::getAuthorisedCategories($user,'com_jevents', 'core.edit.own');
			...
			{
				$authdata_coreedit[$key] = self::authoriseCategories('core.edit', $key, $user);
			}
			...
			else if ($user->id > 0 && $row->created_by() == $user->id)
			{
				...
				{
					$authdata_editown[$key] = self::authoriseCategories('core.edit.own', $key, $user);
				}
	...
	function isEventEditor()
	{
		...
		if (!isset($isEventEditor))
		{
			...
			if (is_null($user))
			{
				...
				if (!$authorisedonly)
				{
					...
					if ($params->get("category_allow_deny", 1) == 0)
					{
						...
						if ($juser->get("id"))
						{
							$okcats = self::getAuthorisedCategories($juser, 'com_jevents', 'core.edit');
							$juser  = Factory::getUser();
							if (count($okcats))
							{
								$dataModel = new \JEventsDataModel();
			...
			/*
			  ...
			  $juser = Factory::getUser();
			  if (self::getGid($user)>=$editorLevel){
			  $isEventEditor = true;
			  }
			  }
			 */
			...
			if (is_null($user))
			{
				...
				if (!$authorisedonly)
				{
					...
					if ($params->get("category_allow_deny", 1) == 0)
					{
						...
						if ($juser->get("id"))
						{
							$okcats = self::getAuthorisedCategories($juser, 'com_jevents', 'core.edit.state');
							$juser  = Factory::getUser();
							if (count($okcats))
							{
								$dataModel = new \JEventsDataModel();
	...
	function canDeleteEvent($row, $user = null)
	{
		...
		// This involes TOO many database queries in Joomla - one per category which can be a LOT
		/*
		  $cats = self::getAuthorisedCategories($user,'com_jevents', 'core.deleteall');
		  if (in_array($row->_catid, $cats))
		  return true;
		 */
		$key = $row->catids() ? json_encode($row->catids()) : json_encode(intval($row->catid()));
		...
		{
			$authdata_coredeleteall[$key] = self::authoriseCategories('core.deleteall', $key, $user);
		}
		...
		if ($row->created_by() == $user->id)
		{
			...
			if (!$authorisedonly && ($publishown == 1 || self::canPublishEvent($row, $user)))
			{
				return true;
			}
		}
	...
	function canPublishEvent($row, $user = null)
	{
		...
		if ($authorisedonly)
		{
			if (!$jevuser)
			{
				...
				{
					$frontendPublish = self::isEventPublisher(false);
		...
		// can publish all?
		if (self::isEventPublisher(true))
		{
			// This involes TOO many database queries in Joomla - one per category which can be a LOT
			/*
			  $cats = self::getAuthorisedCategories($user,'com_jevents', 'core.edit.state');
			  if (in_array($row->_catid, $cats))
			  return true;
			 */
			// allow multi-categories
			$key                      = $row->catids() ? json_encode($row->catids()) : json_encode(intval($row->catid()));
			$authdata_editstate[$key] = self::authoriseCategories('core.edit.state', $key, $user);
		...
		else if ($row->created_by() == $user->id)
		{
			...
			$isEventPublisher = self::isEventPublisher();
			if ($isEventPublisher)
				return true;
			...
			{
				$publishown = self::canPublishOwnEvents($row->ev_id());
				if ($publishown)
				{
					return true;
				}
	...
	function canPublishOwnEvents($evid, $vevent = false)
	{
		...
		if (!$authorisedonly && $publishown)
		{
			...
			if ($evid == 0 && $publishown == 2)
			{
				if ($params->get("category_allow_deny", 1) == 0)
				{
					$okcats = self::getAuthorisedCategories($user, 'com_jevents', 'core.edit.state.own');
	...
	function getContact($id, $attrib = 'Object')
	{
		...
		if (!isset($rows[$id]))
		{
			...
			$query     = "SELECT ju.id, ju.name, ju.username, ju.sendEmail, ju.email, cd.name as contactname, "
				...
				. "\n WHERE block ='0'"
				. "\n AND cd.published =1 "
				. "\n AND cd.access  " . ' IN (' . self::getAid($user) . ')'
				. "\n AND cat.access  " . ' IN (' . self::getAid($user) . ')'
				. "\n AND ju.id = " . $id;
	...
	function componentStylesheet($view, $filename = 'events_css.css')
	{
		...
		{
			self::stylesheet($filename, 'templates/' . Factory::getApplication()->getTemplate() . '/html/' . JEV_COM_COMPONENT . '/' . $view->jevlayout . "/assets/css/");
		}
		else
		{
			self::stylesheet($filename, 'components/' . JEV_COM_COMPONENT . "/views/" . $view->jevlayout . "/assets/css/");
		}
	...
	function loadCustomCSS()
	{
		...
		{
			self::stylesheet('jevcustom.css', 'components/' . JEV_COM_COMPONENT . '/assets/css/');
		}
	}
...
SCRIPT;
	...
	function iCalMailGenerator($row, $n_extras, $ics_method = "PUBLISH")
	{
		...
		if (!empty($icalEvents))
		{
			...
			ob_start();
			self::onDisplayCustomFieldsMultiRow($icalEvents);
			ob_end_clean();
	...
	function getMaxYear()
	{
		...
		$maxyear = self::getYearNumber($maxyear);
	...
	public static function setMenuFilter($key, $value)
	{
		...
		$menufilters       = self::getMenuFilters();
    ...
    public static function setUpdateUrls()
    {
        ...
        {
            self::setUpdateUrlsByPackage($package, $updatesite, $updateJeventsSite);
        }

        $time_end = microtime(true);
        // echo  "self::setUpdateUrls = ".round($time_end - $starttime, 4)."<br/>";
    }
	...
	LEFT JOIN #__update_sites as us on us.update_site_id=map.update_site_id");
        ...
        if ($pkg == 'pkg_jevents')
        {
            ...
            if ($pkgupdate && $pkgupdate->update_site_id !== $updateJeventsSite->update_site_id) {
                ...
                self::setPackageUpdateUrl($pkgupdate, $updateJeventsSite);
            ...
            if ($pkgupdate && $pkgupdate->update_site_id !== $updatesite->update_site_id) {
                // Now update package update URL
                self::setPackageUpdateUrl($pkgupdate, $updatesite);
            } // we have a package but not an update record
            else if ($pkgupdate && $pkgupdate->extension_id && !$pkgupdate->update_site_id) {
                // Now set package update URL
                self::setPackageUpdateUrl($pkgupdate, $updatesite);
    ...
    function dateFromStrftimeFormat($strftime, $datetime = null)
    {
        ...
        $format = self::mapStrftimeFormatToDateFormat($strftime);
