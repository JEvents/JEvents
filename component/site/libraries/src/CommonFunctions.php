...
/**
 * Common utility methods shared by the component and modules.
 * Legacy class name: JEV_CommonFunctions
 */
class CommonFunctions
{
	...
	public static function loadJEventsViewLang()
	{
		...
		$jEventsView = static::getJEventsViewName();
	...
	public static function getJEventsViewName()
	{
		...
		if (!isset($jEventsView))
		{
			...
			// security check
			if (!in_array($jEventsView, static::getJEventsViewList()))
			{
				$jEventsView = "flat";
			}
		}
	...
	public static function setColours($row)
	{
		...
		{
			return array(static::setColor($row));
		}

		static $catData;
		if (!isset($catData)) $catData = static::getCategoryData();
	...
	public static function jEventsLinkCloaking($url = '', $text = '', $attribs = array())
	{
		...
		if ($linkCloaking)
		{
			...
			return static::jEventsDoLink("", $text, array_merge($cloakattribs, $attribs));
		}
		else
		{
			return static::jEventsDoLink(Route::_($url), "$text", $attribs);
		}
	}
