	...
	<div class="export_form">
		<form id="ical" name="ical" method="post" class="<?php isset($_POST['submit']) ? 'icalexportresults' : ''; ?>">
			...
			<div class='choosecat' style='float:left;width:300px;'>
				...
				<div id='othercats' <?php echo $checked ? 'style="display:none;max-height:100px;overflow-y:auto;"' : 'style="display:block;"'; ?> >
					...
					foreach ($categories AS $c)
					{
						...
						else if ($input->get('categories', 0, 'array') && in_array($c->id, $input->post->get('categories', '', null)))
						{
							$cb = $cb . " CHECKED";
						}
			...
			<div class='chooseyear' style='float:left;width:300px;'>
				...
				else if ($input->get('years', 0) && in_array(0, $input->post->get('years', '', null)))
				{
					$yt      = $yt . " CHECKED";
					$checked = true;
				}
				...
				<div id='otheryears' <?php echo $checked ? 'style="display:none;max-height:100px;overflow-y:auto;"' : ''; ?> >
					...
					foreach ($year AS $y)
					{
						...
						else if ($input->get('years', 0) && in_array($y, $input->post->get('years', '', null)))
						{
							$yt = $yt . " CHECKED";
						}
						$yt = $yt . " />" . $y . "<br/>\n";
